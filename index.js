  ...
  try {
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          const publishedLessons = await lessonsCollection.countDocuments({
            status: "Reviewed",
          });
          ...
          const userGrowthRaw = await usersCollection
            .aggregate(monthlyGrowthPipeline)
            .toArray();
          ...
          const rawCategories = await lessonsCollection
      ...
      async (req, res) => {
        try {
          ...
          if (result.modifiedCount > 0 || result.matchedCount > 0) {
            res
              .status(200)
              .send({ success: true, message: "Lesson marked as reviewed!" });
          } else {
            res
              .status(400)
              .send({ success: false, message: "No changes made." });
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          const statsResult = await lessonsCollection
          ...
          const rawGraphData = await lessonsCollection
            .aggregate(graphDataPipeline)
            .toArray();
    ...
    app.post("/api/lessons/save", async (req, res) => {
      try {
        ...
        if (existingSave) {
          return res
            .status(400)
            .send({ success: false, message: "Lesson already saved!" });
        }
        ...
        if (userPlan === "user_free") {
          ...
          if (count >= 5) {
            return res.status(403).send({
              success: false,
              message:
                "Free tier limit reached! You cannot save more than 5 lessons.",
            });
          }
        }
    ...
    app.get("/my-saved-lessons", verifyToken, verifyUser, async (req, res) => {
      try {
        ...
        const savedLessons = await savedCollection
          .aggregate([
            { $match: { userId: userId } },
            {
              $addFields: {
    ...
    app.delete(
      "/api/admin/reports/delete-lesson/:lessonId",
      async (req, res) => {
        try {
          const { lessonId } = req.params;
    ...
    app.post("/api/payments", verifyToken, async (req, res) => {
      try {
        ...
        if (!data.email || !data.planId) {
          return res
            .status(400)
            .send({ success: false, message: "Missing email or planId" });
        }
